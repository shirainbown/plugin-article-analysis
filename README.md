...
- **趋势图 / 数据列表**：全站每日浏览量、访客数、访问次数、跳出率、平均访问时长，支持近 7/30/90 天或自定义起止日期（需先配置 Umami，见下文）。
- **单篇文章分析**：全部 / 已发布 / 草稿 / 待审核文章列表，支持关键词搜索、分类筛选、列头排序、分页；行内「数据」打开单篇详情抽屉（累计数据 + Umami 趋势/明细），「编辑」跳 console 编辑器，「查看」跳前台页面。
